...
SELECT Nome, Data, Local, Descricao, Orçamento FROM Evento;

-- Selecionar todas as tarefas com status 'Pendente'
SELECT Descricao, Prazo_Inicial, Prazo_Final FROM Tarefa WHERE Status = 'Pendente';

-- Buscar os fornecedores que têm disponibilidade
SELECT Nome, Email, Servico FROM Fornecedor WHERE Disponibilidade = 'disponivel';

-- Contar o número de tarefas associadas a um evento específico 
SELECT COUNT(*) FROM Tarefa WHERE FK_Id_Evento = (SELECT Id_Evento FROM Evento WHERE Nome = 'event name');

-- Listar todos os eventos que ocorrem em um local específico
SELECT Nome, Data, Descricao FROM Evento WHERE Local = 'local name';

-- Selecionar os detalhes das propostas que foram enviadas para um evento específico
SELECT Valor, Status, Data FROM Proposta WHERE FK_Id_Fornecedor = (SELECT Id_Fornecedor FROM Fornecedor WHERE Nome = 'Nome do fornecedor');
...
SELECT Nome, Orçamento FROM Evento WHERE Orçamento > 50000;
...
SELECT Nome, Local, Descricao FROM Evento WHERE Data = '2024-08-15';

-- Listar todos os eventos e o número de tarefas associadas a cada um
SELECT Nome, (SELECT COUNT(*) FROM Tarefa WHERE FK_Id_Evento = Evento.Id_Evento) AS Total_Tarefas FROM Evento;
...
SELECT Nome, (SELECT SUM(Valor) FROM Proposta WHERE FK_Id_Evento = Evento.Id_Evento) AS Valor_Total_Propostas FROM Evento;

-- Selecionar todos os fornecedores que oferecem um serviço específico
SELECT Nome, Email FROM Fornecedor WHERE Servico = 'Serviço Específico';

-- Contar o número de propostas recebidas por cada fornecedor
SELECT Nome, (SELECT COUNT(*) FROM Proposta WHERE FK_Id_Fornecedor = Fornecedor.Id_Fornecedor) AS Total_Propostas FROM Fornecedor;
...
SELECT Valor, Data FROM Proposta WHERE Status = 'Aprovada' ORDER BY Valor DESC;
